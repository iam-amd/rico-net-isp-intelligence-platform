import { useEffect, useState, useCallback } from 'react';
import { fetchBandwidth } from '../../api/noc';
import type { BandwidthSummary } from '../../types/noc';

interface BandwidthBar {
  downstream: number;
  upstream: number;
  timestamp: number;
}

export default function BandwidthChart() {
  const [history, setHistory] = useState<BandwidthBar[]>([]);
  const [current, setCurrent] = useState<BandwidthSummary | null>(null);

  const poll = useCallback(async () => {
    try {
      const data = await fetchBandwidth();
      setCurrent(data);

      if (data.data_is_stale || data.downstream_mbps == null || data.upstream_mbps == null) {
        return;
      }

      setHistory(prev => {
        const next = [...prev, {
          downstream: data.downstream_mbps!,
          upstream: data.upstream_mbps!,
          timestamp: Date.now(),
        }];
        return next.slice(-24);
      });
    } catch {
      setCurrent({
        downstream_mbps: null,
        upstream_mbps: null,
        per_port: [],
        sample_window_seconds: 0,
        data_is_stale: true,
        sample_age_seconds: null,
        source_status: 'error',
        message: 'Cannot reach backend bandwidth endpoint.',
      });
    }
  }, []);

  useEffect(() => {
    poll();
    const id = setInterval(poll, 10_000);
    return () => clearInterval(id);
  }, [poll]);

  const maxVal = Math.max(1, ...history.map(b => Math.max(b.downstream, b.upstream)));
  const blocked = current?.data_is_stale || current?.downstream_mbps == null || current?.upstream_mbps == null;
  const message = current?.message || 'Live bandwidth is unavailable from the OLT collector.';

  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Live Bandwidth</span>
        <span className={`text-[9px] ${blocked ? 'text-red-300' : 'text-slate-500'}`}>
          {blocked ? 'blocked' : '10s rolling'}
        </span>
      </div>

      <div className={`flex items-end gap-px h-16 mb-2 rounded-lg border ${
        blocked ? 'border-red-500/40 bg-red-950/20' : 'border-transparent'
      }`}>
        {blocked ? (
          <div className="flex-1 flex flex-col items-center justify-center px-3 text-center">
            <span className="text-[10px] font-bold uppercase tracking-wider text-red-300">
              No live bandwidth sample
            </span>
            <span className="mt-1 text-[9px] leading-tight text-red-200/80">
              {message}
            </span>
          </div>
        ) : history.length === 0 ? (
          <div className="flex-1 flex items-center justify-center text-slate-600 text-xs">Waiting for live sample</div>
        ) : (
          history.map((bar, i) => (
            <div key={`${bar.timestamp}-${i}`} className="flex-1 flex flex-col items-center gap-px justify-end h-full">
              <div
                className="w-full rounded-t-sm bg-blue-500/80"
                style={{ height: `${(bar.downstream / maxVal) * 48}px` }}
              />
              <div
                className="w-full rounded-t-sm bg-green-500/70"
                style={{ height: `${(bar.upstream / maxVal) * 48}px` }}
              />
            </div>
          ))
        )}
      </div>

      <div className="flex gap-4 text-[10px] mb-1">
        <span className={blocked ? 'text-red-300' : 'text-blue-400'}>
          Down <span className="font-mono font-bold">{blocked ? 'NO LIVE' : current?.downstream_mbps?.toFixed(0) ?? '-'}</span>
          {!blocked && ' Mbps'}
        </span>
        <span className={blocked ? 'text-red-300' : 'text-purple-400'}>
          Up <span className="font-mono font-bold">{blocked ? 'NO LIVE' : current?.upstream_mbps?.toFixed(0) ?? '-'}</span>
          {!blocked && ' Mbps'}
        </span>
      </div>
      <div className="text-[9px] text-slate-600 leading-tight">
        EPON .100 exposes per-ONU counters. GPON .200/.210 show PON-level traffic only until reliable counters are mapped.
      </div>
    </div>
  );
}
