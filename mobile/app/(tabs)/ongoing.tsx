...
const STATUS_FILTER: TicketStatus[] = ['Ongoing'];

export default function OngoingScreen() {
    const { tickets, loading, refreshing, onRefresh } = useTicketList({ statusFilter: STATUS_FILTER, pollingInterval: 60000 });
    ...
    return (
        <View style={[styles.container, { backgroundColor: C.background }]}>
            <GradientHeader title="Active Jobs" count={tickets.length} />
            <FlatList
                data={tickets}
                ...
                ListEmptyComponent={() => (
                    <View style={styles.emptyContainer}>
                        <Ionicons name="cafe-outline" size={56} color={C.text.light} />
                        <Text style={[styles.emptyTitle, { color: C.text.primary }]}>No Active Jobs</Text>
                        <Text style={[styles.emptySubtitle, { color: C.text.secondary }]}>
                            You don't have any jobs in progress. Start one from the Pending tab.
                        </Text>
                    </View>
                )}
            />
